...
@section('title', 'Mülk Tipi Düzenle')
@section('breadcrumb_parent', 'Mülk Tipleri')
@section('breadcrumb_title', 'Mülk Tipi Düzenle')
        ...
        <div class="col-md-7">
            <div class="card mb-4">
                <div class="card-header d-flex align-items-center justify-content-between">
                    <h5 class="mb-0">Mülk Tipi Bilgilerini Güncelle</h5>
                    <small class="text-muted float-end">* Zorunlu alanlar</small>
                </div>
                <div class="card-body">
                    <form action="{{ route('admin.property-types.update', $propertyType->id) }}" method="POST" id="propertyTypeForm">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="mb-4">
                            ...
                            <div class="input-group input-group-merge">
                                ...
                                <input type="text" id="name" name="name" class="form-control @error('name') is-invalid @enderror" value="{{ old('name', $propertyType->name) }}" placeholder="Örn: Villa, Otel, Daire, Bungalov" required autofocus />
                            </div>
                            @error('name')
                        ...
                        <div class="mb-4">
                            ...
                            <div class="input-group input-group-merge">
                                ...
                                <input type="text" id="icon" name="icon" class="form-control @error('icon') is-invalid @enderror" value="{{ old('icon', $propertyType->icon ?? 'bx-building-house') }}" placeholder="Örn: bx-building veya bx-home-alt" />
                            </div>
                        ...
                        <div class="mb-4 p-3 bg-lighter rounded border">
                            <div class="form-check form-switch d-flex align-items-center justify-content-between ps-0">
                                ...
                                <input class="form-check-input ms-2" type="checkbox" id="has_rooms" name="has_rooms" value="1" {{ old('has_rooms', $propertyType->has_rooms) ? 'checked' : '' }} style="width: 2.5em; height: 1.3em;">
                            </div>
                        </div>
                        ...
                        <div class="d-flex justify-content-end gap-2 pt-2">
                            ...
                            <button type="submit" class="btn btn-primary">
                                <i class="bx bx-refresh me-1"></i> Güncelle
                            </button>
                        </div>
                    </form>
                </div>
                ...
                <div class="card-body">
                    ...
                    <div class="avatar avatar-xl mx-auto mb-3">
                        <span class="avatar-initial rounded-circle bg-label-primary fs-1" id="icon-preview-box">
                            <i class="bx {{ old('icon', $propertyType->icon ?? 'bx-building-house') }}" id="icon-preview"></i>
                        </span>
                    </div>
                    <p class="card-text fw-bold text-primary mb-0" id="icon-name-display">{{ old('icon', $propertyType->icon ?? 'bx-building-house') }}</p>
                </div>
            </div>

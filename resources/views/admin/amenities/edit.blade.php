...
@section('title', 'Olanak Düzenle')
@section('breadcrumb_parent', 'Olanaklar')
@section('breadcrumb_title', 'Olanak Düzenle')
...
<div class="container-xxl flex-grow-1 container-p-y">
    ...
    <div class="d-flex align-items-center justify-content-between mb-4">
        <h4 class="fw-bold py-1 mb-0">
            <span class="text-muted fw-light">Olanaklar /</span> Olanak Düzenle
        </h4>
        ...
        <div class="col-md-7">
            <div class="card mb-4">
                <div class="card-header d-flex align-items-center justify-content-between">
                    <h5 class="mb-0">Olanak Bilgilerini Güncelle</h5>
                    ...
                    <form action="{{ route('admin.amenities.update', $amenity->id) }}" method="POST" id="amenityForm">
                        ...
                        <div class="mb-4">
                            ...
                            <div class="input-group input-group-merge">
                                ...
                                <input type="text" id="name" name="name" class="form-control @error('name') is-invalid @enderror" value="{{ old('name', $amenity->name) }}" placeholder="Örn: Kablosuz İnternet (Wi-Fi)" required autofocus />
                            </div>
                            @error('name')
                        ...
                        <div class="mb-4">
                            ...
                            <div class="input-group input-group-merge">
                                ...
                                <input type="text" id="icon" name="icon" class="form-control @error('icon') is-invalid @enderror" value="{{ old('icon', $amenity->icon ?? 'bx-wifi') }}" placeholder="Örn: bx-wifi veya bx-pool" />
                            </div>
                            <small class="text-muted">Boxicons kodu yazabilirsiniz. Örn: <code>bx-wifi</code>, <code>bx-swim</code>, <code>bx-car</code></small>
                            @error('icon')
                                <div class="invalid-feedback d-block">{{ $message }}</div>
                            @enderror
                        </div>
                ...
                <div class="card-body">
                    ...
                    <div class="avatar avatar-xl mx-auto mb-3">
                        <span class="avatar-initial rounded-circle bg-label-primary fs-1" id="icon-preview-box">
                            <i class="bx {{ old('icon', $amenity->icon ?? 'bx-wifi') }}" id="icon-preview"></i>
                        </span>
                    </div>
                    <p class="card-text fw-bold text-primary mb-0" id="icon-name-display">{{ old('icon', $amenity->icon ?? 'bx-wifi') }}</p>
                </div>
            </div>
